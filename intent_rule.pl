...
rule('know', 'admission', Return) :-
    findall(X, admission_doc(X), Return).

%ask about all test
rule('provide', 'tests', Return) :-
    findall(X, admission_test(X), Return).

%ask specific about each test

%syllabus
%ask about year sem subject


%ask about all track

%ask about specific track
rule(_, 'ai', Return) :-
    findall(Subject, subject(Subject, _, 'Artificial Intelligence'), Return).

rule(_, 'iot', Return) :-
    findall(Subject, subject(Subject, _, 'Industrial IoT'), Return).

rule(_, 'metaverse', Return) :-
    findall(Subject, subject(Subject, _, 'Metaverse'), Return).

%ask about all syllabus
rule('know', 'syllabus', Return) :-
    findall(Subject, subject(Subject, _, _), Return).
