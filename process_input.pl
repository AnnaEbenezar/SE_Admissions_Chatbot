...
is_punctuation(Char) :-
    member(Char, ['.', ',', '!', '?', ';', ':', '-', '"', '\'']).
is_verb(Word) :-
    member(Word, ['know', 'apply', 'study', 'work', 'specialized', 'provide', 'enroll', 'tell', 'learn', 'prepare']).
is_noun(Word) :-
    member(Word, ['software', 'se', 'admission', 'requirement', 'curriculum', 'syllabus', 'track', 'program', 'exchange', 'dual', 'degree', 'documents']).
...
%synnonym
%know, study, apply, work, specialize
synonym('tell', 'know').
synonym('provide', 'know').
synonym('learn', 'study').
synonym('enroll', 'apply').

%se, syllabus, admission
synonym('software', 'se').
synonym('curriculum', 'syllabus').
synonym('requirement', 'admission').
